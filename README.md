...
| N | Story ID | Title | Description | Acceptance Criteria | Epic | Story Points |
| ----- | :---- | :---- | :---- | :---- | :---: | :---: |
| 1 | US01 | Registrarse como residente | Como residente, quiero registrarme en el sistema proporcionando mis datos personales, credenciales y mi departamento asociado, para poder acceder a la aplicación móvil y gestionar el registro y seguimiento de mis visitas dentro del edificio. | AC01: Registro exitoso de residente:                                   Dado que el usuario se encuentra en el proceso de registro como residente, cuando proporciona un nombre completo válido, un correo electrónico con formato correcto que no ha sido previamente registrado en el sistema, una contraseña que cumple con las políticas de seguridad definidas y selecciona un departamento existente dentro del sistema, entonces el sistema crea un nuevo perfil de usuario con rol residente, almacena la información de manera persistente y habilita el acceso al sistema con una sesión activa. AC02: Validación de datos obligatorios: Dado que el usuario no completa uno o más de los campos obligatorios (nombre, correo electrónico, contraseña o departamento), cuando intenta finalizar el registro, entonces el sistema no procesa la solicitud e indica que debe completar la información faltante. | 1 | 3 |
| 2 | US02 | Registrarse como Portero | Como Portero, quiero registrarme en el sistema proporcionando mis credenciales, para poder acceder al sistema web y gestionar la validación de visitantes. | AC01: Registro exitoso de Portero Dado que el usuario proporciona un nombre completo válido, un correo electrónico único y una contraseña válida, cuando completa el proceso de registro, entonces el sistema crea un perfil de usuario con rol Portero, almacena los datos y le permite acceder al sistema. AC02: Validación de datos obligatorios Dado que el usuario no completa todos los datos requeridos, cuando intenta registrarse, entonces el sistema no permite la creación del perfil e indica que debe completar la información requerida. | 1 | 3 |
| 3 | US03 | Iniciar sesión en el sistema | Como usuario, quiero iniciar sesión en el sistema utilizando mis credenciales registradas, para acceder a las funcionalidades correspondientes a mi rol. | AC01: Inicio de sesión exitoso Dado que el usuario se encuentra registrado y tiene un perfil activo, cuando ingresa un correo electrónico válido asociado a su cuenta y una contraseña correcta, entonces el sistema autentica su identidad, inicia una sesión activa y le permite acceder a las funcionalidades correspondientes a su rol. AC02: Validación de credenciales incorrectas Dado que el usuario ingresa un correo electrónico registrado con una contraseña incorrecta, cuando intenta iniciar sesión, entonces el sistema rechaza la autenticación y no permite el acceso. | 1 | 2 |
| 4 | US04 | Cerrar sesión en el sistema | Como usuario, quiero cerrar sesión para finalizar mi acceso al sistema de forma segura. | AC01: Cierre de sesión exitoso Dado que el usuario tiene una sesión activa en el sistema, cuando solicita cerrar sesión, entonces el sistema finaliza la sesión activa y restringe el acceso a funcionalidades protegidas. AC02: Acceso restringido tras cierre de sesión Dado que el usuario ha cerrado sesión, cuando intenta acceder a funcionalidades que requieren autenticación, entonces el sistema no permite el acceso hasta que el usuario inicie sesión nuevamente. | 1 | 1 |
| 5 | US05 | Editar datos del perfil | Como usuario, quiero actualizar la información de mi perfil para mantener mis datos personales correctos y actualizados dentro del sistema. | AC01: Actualización exitosa del perfil Dado que el usuario modifica uno o más datos de su perfil con información válida, cuando guarda los cambios, entonces el sistema actualiza el perfil de usuario y almacena la nueva información correctamente. AC02: Validación de datos inválidos Dado que el usuario ingresa datos incorrectos o incompletos (por ejemplo, un correo con formato inválido), cuando intenta guardar los cambios, entonces el sistema rechaza la actualización y mantiene los datos previamente registrados. | 1 | 2 |
| 6 | US06 | Registrar una visita | Como residente, quiero registrar los datos de una visita en el sistema, para que el Portero tenga la información anticipada y pueda gestionar el ingreso del visitante. | AC01: Registro exitoso de visita Dado que el residente se encuentra autenticado en el sistema, cuando ingresa el nombre completo del visitante, su DNI y la fecha de la visita, entonces el sistema crea el registro de la visita, lo asocia automáticamente al residente y lo deja disponible para la consulta del Portero. AC02: Validación de datos obligatorios Dado que el residente intenta registrar una visita sin completar los campos obligatorios (nombre o DNI), cuando envía la solicitud, entonces el sistema no registra la visita e indica que los datos obligatorios no han sido completados. | 2 | 3 |
| 7 | US07 | Ingresar y verificar datos del visitante | Como Portero, quiero verificar la información del visitante utilizando los datos previamente registrados en el sistema, para confirmar su identidad antes de permitir el acceso. | AC01: Verificación correcta de identidad Dado que existe una visita registrada en el sistema, cuando el visitante proporciona su nombre y DNI y estos coinciden con los datos registrados, entonces el sistema permite confirmar que la identidad del visitante es válida. AC02: Verificación fallida de identidad Dado que el visitante proporciona datos que no coinciden con ninguna visita registrada, cuando el Portero realiza la verificación, entonces el sistema no permite validar la identidad y mantiene la visita en estado pendiente. | 2 | 3 |
| 8 | US08 | Asociar visita a departamento automáticamente | Como Developer, quiero que el sistema asocie automáticamente cada visita al departamento del residente que la registró, para garantizar la correcta identificación del destino del visitante en el historial. | AC01: Asociación automática al registrar visita Dado que el sistema recibe una solicitud de registro de visita de un residente autenticado, cuando la solicitud es procesada correctamente, entonces el sistema vincula el identificador del departamento del residente al registro de la visita antes de almacenarlo. AC02: Exposición del departamento en la respuesta Dado que el sistema recibe una solicitud de consulta de una visita registrada, cuando la petición es válida y el registro existe, entonces la respuesta debe incluir el identificador y nombre del departamento asociado como parte de los datos de la visita. | 2 | 2 |
| 9 | US09 | Gestionar visita registrada | Como residente, quiero poder editar o eliminar una visita registrada antes de su llegada, para mantener la información actualizada o evitar confusiones si el visitante ya no asistirá. | AC01: Edición exitosa de visita pendiente Dado que existe una visita registrada que aún no ha sido procesada, cuando el residente actualiza los datos del visitante con información válida, entonces el sistema guarda los cambios y refleja la información actualizada en el registro.  AC02: Eliminación exitosa de visita pendiente Dado que existe una visita registrada que aún no ha sido procesada, cuando el residente solicita eliminarla, entonces el sistema elimina el registro y deja de incluirlo en la lista de visitas. AC03: Restricción de modificación sobre visitas procesadas Dado que una visita ya ha sido procesada (aprobada o rechazada), cuando el residente intenta editarla o eliminarla, entonces el sistema rechaza la acción y mantiene el registro sin cambios. | 2 | 3 |
| 10 | US10 | Activar cámara automáticamente | Como Portero, quiero que la cámara del dispositivo IoT se active automáticamente al detectar presencia, para agilizar el inicio del proceso de validación de acceso sin necesidad de activación manual. | AC01: Activación por sensor de movimiento Dado que el dispositivo de captura se encuentra en estado de espera, cuando ocurre un evento de detección de movimiento en la puerta, entonces el sistema debe activar la cámara de forma inmediata para iniciar la transmisión. AC02: Estado de disponibilidad del dispositivo Dado que la cámara ha sido activada por un evento de movimiento, cuando el sistema confirma el encendido del dispositivo IoT, entonces se debe habilitar la generación del registro de visitante. | 3 | 5 |
| 11 | US11 | Capturar imagen del visitante | Como Portero, quiero recibir una fotografía clara de la persona que se encuentra en el ingreso, para contar con información visual del visitante que facilite el proceso de validación y la decisión de acceso. | AC01: Captura automática de imagen tras activación Dado que la cámara ha sido activada por un evento de detección de movimiento, cuando el sistema detecta condiciones adecuadas de estabilidad en la imagen (enfoque mínimo y presencia persistente), entonces debe capturar automáticamente una fotografía del visitante. AC02: Almacenamiento de la imagen capturada Dado que la imagen ha sido capturada exitosamente, cuando el sistema procesa el archivo generado, entonces debe almacenarlo en el sistema como parte del registro del evento de visita, asociándolo al dispositivo, fecha y hora correspondientes. | 3 | 5 |
| 12 | US12 | Grabar audio del visitante | Como Portero, quiero que el sistema capture el audio del entorno durante el proceso de verificación de identidad, para obtener mayor contexto sonoro y mejorar la precisión de la validación de acceso. | AC01: Inicio de grabación de audio Dado que se ha iniciado un evento de captura, cuando la cámara comienza a registrar imagen, entonces el sistema debe activar el micrófono para grabar un clip de audio del visitor. AC02: Calidad y duración del audio Dado que el sistema está grabando, cuando finaliza el tiempo preconfigurado de captura, entonces el archivo de audio debe ser procesado y adjuntado al registro de visita asegurando que sea audible para el Portero. | 3 | 5 |
| 13 | US13 | Transmitir datos del evento al servidor central | Como Developer, quiero que el dispositivo IoT transmita automáticamente al servidor los datos capturados del visitante, para que el sistema registre un nuevo evento de visita disponible en tiempo real para el Portero. | AC01: Recepción exitosa del paquete de datos del dispositivo Dado que el dispositivo IoT envía una solicitud al endpoint de registro de eventos con los datos del visitante (imagen, audio e información del dispositivo), cuando el servidor recibe y valida el paquete, entonces responde con un código de éxito y almacena el evento como visita en estado "Pendiente". AC02: Generación automática de visita pendiente Dado que el servidor ha procesado correctamente los datos enviados por el dispositivo IoT, cuando el evento queda almacenado, entonces el sistema genera automáticamente un registro de visita en estado "Pendiente" asociado al edificio correspondiente, disponible para consulta inmediata. | 3 | 5 |
| 14 | US14 | Asociar captura a una visita | Como Portero, quiero que la información del visitante se vincule automáticamente a los datos de la visita registrada, para consultar el historial de visitas de manera íntegra y organizada. | AC01: Vinculación con registro de visita previa Dado que existe una visita pre-registrada por un residente para ese horario, cuando se procesa la visita pendiente, entonces el sistema debe asociar automáticamente la información del visitante a dicho registro. AC02: Integridad en el historial de accesos Dado que la validación ha concluido, cuando se genera el registro de acceso, entonces la infomración del visitante (foto y audio) debe quedar almacenada permanentemente dentro del historial de visita asociado al departamento correspondiente. | 3 | 3 |
| 15 | US15 | Visualizar visitantes en espera | Como Portero, quiero visualizar en tiempo real una lista de visitantes que se encuentran en espera de validación, para poder gestionar el orden de atención y tomar decisiones de acceso de manera organizada. | AC01: Registro automático de visitante en espera Dado que el dispositivo IoT detecta la presencia de un visitante y envía la información al servidor, cuando el sistema procesa correctamente el evento, entonces debe generar un registro en estado "Pendiente" que represente al visitante en espera de validación. AC02: Visualización de lista de visitantes pendientes Dado que existen visitantes en estado "Pendiente", cuando el Portero solicita consultar los visitantes en espera, entonces el sistema retorna la lista actualizada de todos los visitantes pendientes de validación. AC03: Orden cronológico de atención Dado que existen múltiples visitantes en espera, cuando el sistema presenta la lista, entonces debe ordenarlos cronológicamente según la hora de llegada, priorizando los más antiguos. | 4 | 3 |
| 16 | US16 | Visualizar foto del visitante | Como Portero o residente, quiero visualizar la imagen capturada del visitante asociada a la solicitud de acceso, para facilitar la identificación visual antes de tomar una decisión. | AC01: Disponibilidad de imagen asociada al evento Dado que el sistema ha capturado una imagen del visitante durante el evento de ingreso, cuando se consulta el registro de la visita, entonces el sistema debe permitir acceder a la imagen almacenada asociada a dicho evento. AC02: Integridad de la imagen visualizada Dado que existe una imagen registrada, cuando el sistema la presenta para su visualización, entonces debe garantizar que el archivo sea accesible y corresponda correctamente al visitante registrado. AC03: Asociación correcta con la visita Dado que el sistema gestiona múltiples visitas, cuando se accede a una solicitud específica, entonces debe mostrar únicamente la imagen correspondiente a ese evento de visitante. | 4 | 2 |
| 17 | US17 | Reproducir audio del visitante | Como Portero o residente, quiero reproducir el audio capturado del visitante durante el evento de ingreso, para escuchar la información proporcionada y apoyar el proceso de validación manual. | AC01: Disponibilidad del audio del visitante Dado que el sistema ha registrado un archivo de audio durante el evento de visita, cuando se consulta el registro correspondiente, entonces el sistema debe permitir acceder al audio asociado al visitante. AC02: Reproducción del audio almacenado Dado que existe un archivo de audio válido, cuando el sistema procesa la solicitud de reproducción, entonces debe reproducir el contenido completo del audio registrado sin alteraciones. AC03: Asociación correcta del audio con la visita Dado que existen múltiples registros de visitas, cuando se accede a un evento específico, entonces el sistema debe reproducir únicamente el audio vinculado a ese visitante. | 4 | 2 |
| 18 | US18 | Comparar datos del visitante con visitas registradas | Como Developer, quiero que el sistema compare los datos capturados del visitante contra las visitas previamente registradas, para clasificar el evento y asistir al Portero en la validación de acceso. | AC01: Respuesta con coincidencia encontrada Dado que el sistema recibe los datos de un visitante capturado por el dispositivo IoT, cuando existe una visita registrada cuyos atributos (nombre o DNI) coinciden con los datos recibidos, entonces el sistema marca el evento como "Coincidente" y lo asocia al registro de visita correspondiente. AC02: Respuesta sin coincidencia Dado que el sistema procesa los datos de un visitante capturado, cuando no encuentra ninguna visita registrada que coincida con los datos recibidos, entonces el sistema clasifica el evento como "Visita no registrada" y mantiene el estado "Pendiente" para validación manual. AC03: Exposición del resultado de comparación en la consulta Dado que el sistema ha procesado la comparación de datos, cuando el Portero realiza una solicitud de consulta del detalle del visitante, entonces la respuesta incluye tanto los datos capturados como el resultado de la comparación (Coincidente / No registrada). | 4 | 5 |
| 19 | US19 | Registrar acceso aprobado de visitante | Como Developer, quiero que el sistema genere automáticamente un registro de acceso cuando un visitante es autorizado, para mantener un historial confiable con trazabilidad completa del evento. | AC01: Generación del registro de acceso aprobado Dado que el sistema recibe una acción de aprobación sobre una visita en estado "Pendiente", cuando la solicitud es procesada correctamente, entonces el sistema genera un registro de acceso que incluye fecha, hora, identificador del visitante, departamento asociado y referencias a la evidencia capturada. AC02: Vinculación de  evidencia al registro Dado que el sistema almacena un registro de acceso aprobado, cuando el evento queda persistido, entonces el registro debe incluir las referencias a la fotografía y el audio capturados durante el evento. AC03: Disponibilidad del registro en el historial Dado que el acceso ha sido registrado, cuando el sistema recibe una solicitud de consulta del historial, entonces el evento debe aparecer con estado "Aprobado" e incluir todos los datos asociados disponibles. | 4 | 3 |
| 20 | US20 | Registrar intento de acceso rechazado | Como Developer, quiero que el sistema registre los eventos de acceso rechazado junto con su evidencia, para mantener trazabilidad de los intentos no autorizados en el historial del edificio. | AC01: Generación del registro de acceso rechazado Dado que el sistema recibe una acción de rechazo sobre una visita en estado "Pendiente", cuando la solicitud es procesada, entonces el sistema genera un registro con estado "Rechazado" que incluye fecha, hora y datos del visitante. AC02: Vinculación de evidencia al registro de rechazo Dado que el sistema almacena un evento de rechazo, cuando el registro queda persistido, entonces debe incluir referencias a la imagen y audio capturados durante el evento para fines de auditoría. AC03: Disponibilidad en el historial Dado que el evento ha sido registrado, cuando el sistema recibe una solicitud de consulta del historial, entonces el intento debe aparecer con estado "Rechazado" junto con los datos asociados. | 4 | 3 |
| 21 | US21 | Sincronizar decisión del residente sobre una visita | Como Portero, quiero que el sistema refleje automáticamente la decisión tomada por el residente sobre una visita, para poder actuar sobre el acceso sin necesidad de comunicación externa. | AC01: Actualización de estado a "Aprobada" Dado que una visita se encuentra en estado "Pendiente", cuando el residente registra una decisión de aprobación desde su aplicación, entonces el sistema actualiza el estado de la visita a "Aprobada" y lo mantiene disponible para consulta del Portero. AC02: Actualización de estado a "Rechazada" Dado que una visita se encuentra en estado "Pendiente", cuando el residente registra una decisión de rechazo, entonces el sistema actualiza el estado de la visita a "Rechazada" sin eliminar el registro. AC03: Disponibilidad inmediata de la decisión Dado que el residente ha tomado una decisión, cuando el Portero consulta la visita, entonces el sistema muestra el estado actualizado para permitir la gestión correspondiente del acceso. AC04: Persistencia del cambio de estado Dado que se ha registrado un cambio de estado, cuando el sistema almacena la información, entonces el nuevo estado permanece registrado para futuras consultas en el historial. | 4 | 3 |
| 22 | US22 | Generar y enviar notificación de llegada al residente | Como Developer, quiero que el sistema genere y envíe automáticamente una notificación al residente cuando se registra la llegada de un visitante, para que pueda tomar una decisión de acceso de manera oportuna. | AC01: Generación de notificación ante nuevo evento de visita Dado que el sistema registra un nuevo evento de visita en estado "Pendiente", cuando el proceso de notificación es ejecutado, entonces el sistema genera una notificación dirigida al residente asociado al departamento de destino. AC02: Enrutamiento correcto de la notificación Dado que el sistema genera una notificación de visita, cuando determina el destinatario, entonces la notificación es enviada únicamente al residente vinculado al departamento correspondiente, sin afectar a otros usuarios. AC03: Registro del resultado del envío Dado que el sistema intenta enviar una notificación, cuando el proceso de envío concluye (exitoso o fallido), entonces el sistema registra el resultado del intento para fines de seguimiento y auditoría. | 5 | 5 |
| 23 | US23 | Mostrar datos del visitante | Como residente, quiero visualizar la información básica del visitante notificado, para poder identificar rápidamente a la persona que solicita acceso. | AC01: Visualización de datos del visitante Dado que el residente recibe una notificación de visita, cuando consulta el detalle de la misma, entonces el sistema debe mostrar los datos disponibles del visitante, incluyendo nombre y documento de identidad si han sido proporcionados. AC02: Identificación de visitante no registrado Dado que el visitante no cuenta con un registro previo en el sistema, cuando el residente consulta la información, entonces el sistema debe indicar que se trata de un visitante no registrado. | 5 | 2 |
| 24 | US24 | Mostrar foto del visitante | Como residente, quiero visualizar la imagen del visitante capturada por el sistema, para poder identificarlo visualmente antes de tomar una decisión sobre su acceso. | AC01: Disponibilidad de imagen del visitante Dado que el sistema ha capturado una imagen del visitante durante el evento de acceso, cuando el residente recibe la notificación de la visita, entonces el sistema debe permitir visualizar la imagen asociada al visitante. AC02: Visualización de la imagen en la consulta de la visita Dado que el residente accede al detalle de la notificación, cuando consulta la información del visitante, entonces el sistema debe mostrar la imagen capturada como parte de la evidencia del evento. | 5 | 2 |
| 25 | US25 | Registrar recepción de notificación por el residente | Como Developer, quiero que el sistema registre el momento en que el residente accede a una notificación de visita, para mantener trazabilidad del estado de atención de cada evento. | AC01: Registro de visualización al acceder a la notificación Dado que el sistema recibe una solicitud de consulta de una notificación por parte del residente destinatario, cuando la petición es válida y la notificación existe, entonces el sistema registra la fecha y hora del acceso y actualiza el estado de la notificación. AC02: Actualización de estado a "Vista" Dado que el sistema ha registrado el acceso del residente a una notificación, cuando el proceso de actualización es ejecutado, entonces el estado de la notificación queda registrado como "Vista" de forma persistente. | 5 | 2 |
| 26 | US26 | Visualizar historial de notificaciones | Como residente, quiero acceder a un historial de notificaciones recibidas, para poder revisar eventos pasados relacionados con visitas y mantener un control personal. | AC01: Visualización de lista de notificaciones Dado que existen notificaciones registradas en el sistema, cuando el residente accede al historial, entonces el sistema debe mostrar una lista de notificaciones ordenadas cronológicamente. AC02: Información incluida en cada notificación Dado que el residente consulta el historial, cuando visualiza una notificación, entonces el sistema debe mostrar información relevante como fecha, hora y datos del visitante. AC03: Visualización del estado de la visita Dado que cada notificación corresponde a una visita, cuando el residente revisa el historial, entonces el sistema debe indicar el estado final de cada visita (aprobada, rechazada o no atendida). | 5 | 3 |
| 27 | US27 | Permitir visualizar detalle del visitante | Como residente, quiero acceder al detalle completo de la visita, para contar con mayor información del visitante antes de tomar una decisión sobre su acceso. | AC01: Acceso al detalle de la visita Dado que el residente ha recibido una notificación de visita, cuando consulta la información asociada al evento, entonces el sistema debe permitir visualizar el detalle completo del visitante. AC02: Visualización de evidencia del visitante Dado que existe información capturada del visitante, cuando el residente accede al detalle, entonces el sistema debe mostrar la imagen y otros datos disponibles como parte del registro. AC03: Disponibilidad de información contextual Dado que el residente consulta el detalle de la visita, cuando revisa la información, entonces el sistema debe mostrar datos relevantes como la fecha, hora y cualquier información registrada del visitante. | 5 | 2 |
| 28 | US28 | Permitir aprobar o rechazar visita | Como residente, quiero poder aprobar o rechazar una visita, para decidir si se permite o no el ingreso del visitante al edificio. | AC01: Registro de decisión del residente Dado que existe una visita en estado “Pendiente”, cuando el residente toma una decisión, entonces el sistema debe registrar la acción como “Aprobada” o “Rechazada”. AC02: Restricción de múltiples decisiones Dado que el residente ya ha tomado una decisión sobre una visita, cuando intenta realizar una nueva acción sobre el mismo evento, entonces el sistema no debe permitir modificar el estado previamente registrado. AC03: Persistencia de la decisión Dado que se ha registrado una decisión, cuando el sistema almacena la información, entonces el estado de la visita debe mantenerse actualizado para futuras consultas. | 5 | 3 |
| 29 | US29 | Detectar presencia en la puerta | Como Portero, quiero que el sistema detecte automáticamente cuando una persona se encuentra frente a la puerta de ingreso, para poder atender oportunamente a los visitantes sin necesidad de estar físicamente en la entrada. | AC01: Detección de presencia activa Dado que el dispositivo IoT se encuentra operativo y conectado al sistema, cuando el sensor detecta la presencia de una persona frente a la puerta de ingreso, entonces el sistema procesa la señal y actualiza el estado del punto de acceso a "Persona en la puerta" en tiempo real. AC02: Ausencia de presencia en la puerta Dado que el sistema ha registrado previamente la presencia de una persona en la puerta, cuando el sensor deja de detectar movimiento durante un periodo continuo mayor a 10 segundos, entonces el sistema actualiza automáticamente el estado del punto de acceso a "Sin presencia en la puerta". | 6 | 5 |
| 30 | US30 | Visualizar estado de la puerta | Como Portero, quiero visualizar en el sistema el estado actual de la puerta (abierta o cerrada), para poder garantizar el control de accesos y la seguridad del edificio en todo momento. | AC01: Visualización de puerta cerrada Dado que el sistema recibe información del sensor de la puerta, cuando la puerta se encuentra en estado cerrado, entonces el sistema expone el estado actual como "Puerta cerrada" y habilita únicamente la acción de apertura para el Portero autenticado. AC02: Visualización de puerta abierta Dado que el sistema recibe información del sensor de la puerta, cuando la puerta se encuentra en estado abierto, entonces el sistema expone el estado actual como "Puerta abierta" y habilita únicamente la acción de cierre para el Portero autenticado. | 6 | 3 |
| 31 | US31 | Registrar eventos de apertura y cierre de la puerta | Como administrador o residente, quiero que el sistema registre automáticamente los eventos de apertura y cierre de la puerta, para contar con un historial confiable que permita auditar los accesos al edificio. | AC01: Registro de apertura de puerta Dado que el sistema se encuentra activo y recibe información del dispositivo, cuando se detecta que la puerta cambia a estado abierto, entonces el sistema genera automáticamente un registro que incluye la fecha y hora del evento, el identificador del dispositivo y el tipo de evento "Apertura de puerta". AC02: Registro de cierre de puerta Dado que el sistema se encuentra activo y recibe información del dispositivo, cuando se detecta que la puerta cambia a estado cerrado, entonces el sistema genera automáticamente un registro que incluye la fecha y hora del evento, el identificador del dispositivo y el tipo de evento "Cierre de puerta". | 6 | 3 |
| 32 | US32 | Visualizar historial de visitas | Como Portero, quiero que el sistema me permita visualizar un registro completo y organizado de todas las visitas (pendientes, aprobadas y rechazadas), para llevar un control claro y confiable de las personas que han ingresado o intentado ingresar al edificio. | AC01: Visualización completa del historial Dado que el Portero se encuentra autenticado en el sistema, cuando solicita consultar el historial de visitas, entonces el sistema retorna una lista cronológica de todas las visitas registradas en el edificio, incluyendo nombre del visitante, DNI, departamento asociado, fecha, hora y estado. AC02: Visualización filtrada por residente Dado que el residente se encuentra autenticado en el sistema, cuando solicita consultar sus visitas, entonces el sistema retorna únicamente las visitas asociadas a su departamento con el estado actual de cada una. AC03: Actualización del estado de la visita según evento físico Dado que una visita se encuentra en estado “Pendiente”, cuando el Portero aprueba el ingreso y el dispositivo IoT confirma la apertura de la puerta mediante el sensor, entonces el sistema debe actualizar automáticamente el estado de la visita a “Aprobada” y registrar un evento de auditoría que incluya la relación entre el visitante, el departamento y el timestamp exacto de apertura. | 6 | 3 |
| 33 | US33 | Modificar datos de autenticación | Como residente, quiero que el sistema me permita actualizar mis credenciales de acceso (nombre de usuario, contraseña y correo electrónico), para mantener la seguridad y vigencia de mi cuenta dentro de la plataforma. | AC01: Actualización de nombre de usuario válida Dado que el residente se encuentra autenticado en el sistema, cuando solicita actualizar su nombre de usuario ingresando un nuevo valor, entonces el sistema debe validar que el nuevo nombre cumpla con las reglas de formato establecidas y que no esté previamente registrado por otro usuario, y en caso de ser válido, debe actualizarlo correctamente en la base de datos. AC02: Actualización de contraseña con validación de identidad Dado que el residente desea modificar su contraseña, cuando ingresa su contraseña actual y una nueva contraseña, entonces el sistema debe verificar que la contraseña actual coincida con la registrada y que la nueva cumpla con los criterios de seguridad definidos, y posteriormente actualizar la contraseña de forma segura. AC03: Actualización de correo electrónico con verificación posterior Dado que el residente solicita cambiar su correo electrónico, cuando ingresa una nueva dirección válida, entonces el sistema debe actualizar el correo en estado “pendiente de verificación” y enviar un mecanismo de validación al nuevo correo para confirmar su autenticidad antes de considerarlo activo. | 7 | 5 |
| 34 | US34 | Recuperar contraseña | Como residente, quiero recuperar el acceso a mi cuenta en caso de olvidar mi contraseña, para poder continuar utilizando el sistema sin perder mi información. | AC01: Generación de token de recuperación válido Dado que el residente no puede acceder a su cuenta, cuando solicita la recuperación de contraseña proporcionando un correo electrónico registrado, entonces el sistema debe validar la existencia de la cuenta asociada y generar un token único de recuperación, enviándolo al correo registrado para permitir el restablecimiento de la contraseña. AC02: Restablecimiento de contraseña mediante  token válido Dado que el residente accede al enlace de recuperación con un token válido, cuando define una nueva contraseña que cumple con las políticas de seguridad, entonces el sistema debe actualizar la contraseña en la base de datos y permitir el acceso posterior con las nuevas credenciales. AC03: Control de expiración y reutilización del token Dado que el token de recuperación tiene un tiempo de validez limitado, cuando el residente intenta utilizar un token expirado o previamente utilizado, entonces el sistema debe rechazar la operación e indicar que debe iniciar nuevamente el proceso de recuperación. | 7 | 5 |
| 35 | US35 | Proteger datos sensibles | Como Developer, quiero almacenar las contraseñas utilizando hashing seguro, para proteger la información de los usuarios. | AC01: Encriptación de credenciales mediante hashing (BCrypt/EksBlowfish) Dado que el sistema procesa la creación o actualización de una cuenta de usuario (residente o Portero), cuando la contraseña es enviada al servidor para su almacenamiento, entonces el sistema debe aplicar un algoritmo de hashing basado en EksBlowfish (BCrypt) utilizando un factor de costo configurado para resistir ataques de fuerza bruta. AC02: Verificación segura y protección contra fugas de datos Dado que el sistema almacena las credenciales de un usuario, cuando se consulta directamente el valor guardado en la base de datos, entonces la contraseña no debe encontrarse en texto plano sino como un hash irreversible. | 7 | 3 |
| 36 | US36 | Validar sesiones y autorización en la plataforma | Como Developer, quiero implementar un mecanismo de validación de sesiones y autorización en cada solicitud del sistema, para garantizar que solo usuarios autenticados y autorizados puedan ejecutar acciones dentro de la plataforma. | AC01: Validación de sesión activa en cada solicitud Dado que el sistema recibe una solicitud desde un cliente, cuando la petición incluye un token de sesión, entonces el sistema debe verificar que el token sea válido, no haya expirado y corresponda a una sesión activa antes de procesar cualquier lógica de negocio. AC02: Rechazo de solicitudes con sesión inválida o inexistente Dado que el sistema recibe una solicitud sin token, con token inválido o expirado, cuando intenta procesar la petición, entonces el sistema debe rechazar la operación, no ejecutar ninguna acción y devolver una respuesta indicando que la autenticación es requerida o inválida. AC03: Validación de  autorización por contexto de negocio Dado que el usuario está autenticado correctamente, cuando intenta acceder a un recurso o ejecutar una acción, entonces el sistema debe validar que el rol del usuario corresponde al nivel de acceso requerido por dicho recurso antes de permitir la operación. | 7 | 5 |
| 37 | US37 | Visualizar información general del sistema | Como persona interesada, quiero visualizar información general del sistema en la página principal, para comprender el propósito de la solución y cómo mejora el control de acceso de visitantes en edificios residenciales. | AC01: Visualización de información principal Dado que accede la persona interesada al sitio web, cuando ingresa a la página principal, entonces el sistema muestra una descripción clara del propósito del sistema y sus principales beneficios. AC02: Disponibilidad del contenido Dado quela persona interesada consulta la página principal, cuando el sistema carga la información, entonces el contenido debe mostrarse correctamente sin omitir información relevante. | 8 | 2 |
| 38 | US38 | Visualizar funcionalidades principales | Como la persona interesada, quiero conocer las funcionalidades principales del sistema, para entender cómo se realiza el registro, validación y monitoreo de visitantes. | AC01: Visualización de funcionalidades Dado que la persona interesada navega por la página informativa, cuando consulta la sección de funcionalidades, entonces el sistema muestra las principales funciones del sistema de control de visitas. AC02: Relación con el servicio Dado que la persona interesada revisa la información del sistema, cuando visualiza las funcionalidades, entonces el contenido debe explicar cómo cada función aporta al control de acceso. | 8 | 2 |
| 39 | US39 | Visualizar beneficios del sistema | Como la persona interesada, quiero visualizar los beneficios del sistema, para comprender el valor que ofrece a residentes y personal del edificio. | AC01: Mostrar beneficios del sistema Dado que la persona interesada accede al sitio web, cuando consulta la sección de beneficios, entonces el sistema muestra las ventajas principales del uso de la solución. AC02: Información comprensible Dado que la persona interesada revisa la sección de beneficios, cuando analiza el contenido, entonces la información debe presentarse de forma clara y comprensible. | 8 | 2 |
| 40 | US40 | Visualizar información de contacto | Como la persona interesada, quiero visualizar información de contacto del proyecto, para poder comunicarme con el equipo responsable del sistema. | AC01: Mostrar datos de contacto Dado que la persona interesada accede al sitio web, cuando consulta la sección de contacto, entonces el sistema muestra la información de contacto disponible del proyecto. AC02: Disponibilidad permanente Dado que la persona interesada desea comunicarse, cuando consulta la información del proyecto, entonces los datos de contacto deben permanecer accesibles desde la página. | 8 | 1 |
| 41 | US41 | Navegar entre secciones informativas | Como la persona interesada, quiero navegar entre las diferentes secciones de la página web, para acceder fácilmente a la información del sistema. | AC01: Navegación entre secciones Dado que la persona interesada se encuentra en la página web, cuando selecciona una sección informativa, entonces el sistema muestra el contenido correspondiente de manera organizada. AC02: Continuidad de navegación Dado que la persona interesada recorre varias secciones, cuando cambia entre contenidos, entonces el sistema mantiene una navegación continua sin pérdida de información. | 8 | 2 |
...
<p align="center">
<img src="https://imgur.com/a/FuzqHI0" alt="3" width="1100">
</p>

<p align="center">
<img src="https://imgur.com/YigLQ6l" alt="3" width="1100">
</p>
